#!/usr/bin/env python
"""Build a B2 (Tester Reliability) protocol from the ORCAS click log.

ORCAS (Open Resource for Click Analysis in Search, Microsoft 2020) is a
public, non-gated dataset of 10.4M Bing queries with 18.8M click-derived
qrels over the MS MARCO document collection. It is the recommended
non-gated alternative to TripClick / TianGong-ST for tester-reliability
benchmarking.

The protocol mirrors ``build_aol_b2.py``: sample N queries that have a
sufficient number of clicked documents, pad each candidate pool with
distractor docs sampled from the global judged-doc set (since ORCAS qrels
are positive-only), and synthesise 10 retrieval systems by ranking pools
with quality alpha in [0.15, 0.80]. Each tester replays its click model
on the ranking; the trusted tester is qrels-nDCG@10 computed directly
from the click-derived relevance.
"""
...
def main():
    ...
    print("Loading ORCAS qrels (full file)...")
    ds = ir_datasets.load("msmarco-document/orcas")
