...
class RankedItem:
    ...
    Attributes:
        ...
        meta: Free-form metadata. Adapters that have surface text such
            as URL, title, or snippet should put them here so simulators
            can render realistic SERPs without re-loading the source
            dataset.
    """
...
class InteractionSession:
    ...
    def validate(self) -> list[str]:
        """Validate session against schema constraints.

        Checks:
        - Click ranks fall within SERP bounds
        - Timestamps are non-decreasing
        - Clicked item identifiers are consistent with SERP items
        ...
        for i in range(1, len(timestamps)):
            if timestamps[i] < timestamps[i - 1]:
                errors.append(
                    f"Timestamps not non-decreasing: event {i} "
                    f"({timestamps[i]}) < event {i-1} ({timestamps[i-1]})"
                )

        # Collect SERP items per query for click validation
        current_serp_items: dict[str, int] = {}  # doc_id -> max_rank
        max_serp_rank = 0

        for event in self.events:
            if event.type == EventType.SERP_VIEW and event.ranked_items:
                current_serp_items = {
                    item.doc_id: item.rank for item in event.ranked_items
                }
                max_serp_rank = max(item.rank for item in event.ranked_items)

            if event.type == EventType.CLICK and event.clicked_items:
                for click in event.clicked_items:
                    # Check click rank within SERP bounds
                    if max_serp_rank > 0 and click.rank > max_serp_rank:
                        errors.append(
                            f"Click rank {click.rank} exceeds SERP max "
                            f"rank {max_serp_rank} in event {event.event_id}"
                        )
                    # Check item ID consistency
                    if current_serp_items and click.doc_id not in current_serp_items:
                        errors.append(
                            f"Clicked doc '{click.doc_id}' not found in "
                            f"current SERP in event {event.event_id}"
                        )
